...
Sprite::Sprite(Surface* a_Surface, unsigned int a_NumFrames) : m_Width(a_Surface->GetWidth() / a_NumFrames),
...
void Sprite::DrawScaled(int a_X, int a_Y, int a_Width, int a_Height, Surface* a_Target)
{
    ...
    //Set end x to within screen
    if (x2 > a_Target->GetWidth()) x_end = a_Width - (x2 - a_Target->GetWidth());
    if (y1 < 0)
    {
        y_start += -y1;
    }
    if (y2 > a_Target->GetHeight()) y_end = a_Height - (y2 - a_Target->GetHeight());
